document.addEventListener('DOMContentLoaded', (): void => {
    ...
    let screenBehavior: boolean = false;
    let fractionInput: boolean = false;

    //МАТЕМАТИЧЕСКИЕ ОПЕРАЦИИ

    // Функция для преобразования строки-дроби в десятичное число number
    const toNumberFraction = (fractionStr: string): number | null => {
        if (fractionStr === '' || fractionStr === '-') return null;
    ...
    const toFraction = (decimal: number): string => {
        ...
        // Сокращение дроби
        const reduction = (a: number, b: number): number => {
    ...
    const calculate = (): void => {
        if (operation === null || screenBehavior) return;

        const prev: number = toNumberFraction(previousOperand);
        const current: number = toNumberFraction(currentOperand);
    ...
    const continueWriting = (number: string): void => {
        if (screenBehavior) {
            currentOperand = '';
            screenBehavior = false;
            fractionInput = false;
    ...
    const calculateSquareRoot = (): void => {
        const numberValue: number | null = toNumberFraction(currentOperand);
    ...
    const display: HTMLElement = document.querySelector('.display') as HTMLElement;
    ...
    buttons.forEach((button: HTMLButtonElement): void => {
        button.addEventListener('click', (): void => {
            ...
            const buttonHTML: string | null = button.innerHTML;
